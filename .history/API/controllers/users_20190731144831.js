...
exports.REGISTER_USER = (req, res) => {
    ...
    User.find({ $or: [{ email: req.body.email }, { cardNumber: req.body.cardNumber }] })
        .exec()
        .then(user => {
            if (user) {
                return res.status(409).json({ email: 'Card already exists' })
            }
            //  if (user.cardNumber === req.body.cardNumber) {
            //     return res.status(409).json({ cardNumber: 'Card Number already exist' })
            // }
